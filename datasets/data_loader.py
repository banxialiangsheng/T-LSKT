...
class KTDataset(Dataset):
    ...
    def __load_data__(self, sequence_path, folds, pad_val=-1):
        ...
        """
        dori = {"qseqs": [], "cseqs": [], "rseqs": [], "tseqs": [], "utseqs": [], "smasks": [], "time_intervals": []}
        ...
        # 检查是否有多知识点时间间隔
        has_multi_concept_intervals = False
        multi_concept_count = 0
        if "time_intervals" in df.columns:
            # 检查第一行的时间间隔值
            for i, row in df.iterrows():
                if "time_intervals" in row and row["time_intervals"]:
                    # 获取第一个时间间隔字符串
                    first_interval = row["time_intervals"].split(",")[0]
                    try:
                        # 尝试将其转换为浮点数
                        float(first_interval)
                        # 如果成功，继续检查下一行
                    except ValueError:
                        # 如果转换失败，可能是因为格式不正确或者是多知识点格式
                        multi_concept_count += 1
                    
                    if multi_concept_count >= 3:  # 如果有多行显示这种模式，认为是多知识点格式
                        has_multi_concept_intervals = True
                        break
        ...
        for i, row in df.iterrows():
            ...
            if "time_intervals" in row:
                if has_multi_concept_intervals:
                    # 处理多知识点时间间隔
                    # 每行的time_intervals是一个字符串列表，每个字符串包含多个知识点的时间间隔
                    intervals_per_step = []
                    for interval_str in row["time_intervals"].split(","):
                        # 将字符串分割成浮点数列表
                        try:
                            intervals = [float(val) for val in interval_str.split(",")]
                            intervals_per_step.append(intervals)
                        except ValueError:
                            # 如果解析失败，添加一个默认值
                            intervals_per_step.append([0.0])
                    dori["time_intervals"].append(intervals_per_step)
                else:
                    # 处理单知识点时间间隔
                    dori["time_intervals"].append([float(_) for _ in row["time_intervals"].split(",")])
        ...
        for key in dori:
            if key == "time_intervals":
                if has_multi_concept_intervals:
                    # 处理多知识点时间间隔
                    # 找出最大的知识点数量
                    max_concepts = max(len(intervals) for seq in dori[key] for intervals in seq)
                    
                    # 创建填充后的张量
                    padded_tensor = []
                    for seq in dori[key]:
                        padded_seq = []
                        for intervals in seq:
                            # 填充到最大知识点数量
                            padded_intervals = intervals + [-1] * (max_concepts - len(intervals))
                            padded_seq.append(padded_intervals)
                        padded_tensor.append(padded_seq)
                    
                    # 转换为张量 [batch, seq_len, num_concepts]
                    dori[key] = FloatTensor(padded_tensor)
                else:
                    # 单知识点情况，保持原有处理方式
                    dori[key] = FloatTensor(dori[key])
            elif key not in ["rseqs"]:  # 除了rseqs和time_intervals之外的其他键
